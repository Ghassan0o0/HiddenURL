...
def makeEvil(char, unicd, uninum, newurl, oldurl, output):
    printParser('\n{0}[{1}*{0}]{1} Domain name: %s\n{0}[{1}*{0}]{1} Char replaced: %s\n{0}[{1}*{0}]{1} Using Unicode: %s\n{0}[{1}*{0}]{1} Unicode number: %s\n{0}[{1}*{0}]{1} Hidden URL: {3}%s{1}'.format(GREEN, END, YELLOW, RED) % (oldurl, char, unicd, uninum, newurl), output)
...
def check_EVIL(url):
    ...
    '''
    Check hidden chars in URL
    :param url: suspicious URL
    :return: result of check and the hidden chars
    '''
    ...
    if result:
        msg = '{0}[{1}*{0}]{1} Hidden URL detected: {2}{3}{1}'.format(GREEN,END,RED,url)
        msg += '\n{0}[{1}*{0}]{1} Unicode characters used: {2}'.format(GREEN,END,result)
    else:
        msg = '{0}[{1}!{0}]{1} Hidden URL NOT detected: {2}'.format(YELLOW, END, url)

    return msg

def urls_list(file, checkConnection, output):
    '''
    Read the file to verify Hidden URL
    :param file: file with a list of Hidden URLs 
    :return: file reading
    '''
...
def parseHandler():
    parser = ArgumentParser(usage="python3 hiddenurl.py [options]", description="Generate unicode hidden domains for IDN Homograph Attack and detect them.")
    parser.add_argument("-g", dest="generate", action="store_true", help="Generate unicode hidden domains")
    ...
    parser.add_argument("-o", dest="output", help="Save generated hidden domains to a file")
